...
export class AppointmentsComponent implements OnInit {
  ...
  constructor(private appointmentsService: AppointmentsService, private doctorService: DoctorService, private alertService: AlertService, public dialog: MatDialog) { }
  ...
  createDiagnostic(diagnostic: newDiagnostic) {
    this.appointmentsService.createDiagnostic(diagnostic).subscribe(
      res => {
        this.alertService.showSuccess("Diagnostic successfully created");
        this.getDoctorAppointments();
      },
      err => {
        console.log(err)
